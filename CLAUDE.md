...
3. **High-Stakes Oversight** - Destructive ops (delete reports) require human confirmation via `interrupt()`
...
│   ├── router.py                    # Classify intent (data_query / general / delete)
...
│   └── delete_reports.py            # Mock report deletion with interrupt() confirmation
├── golden_knowledge/
...
└─────────────┘
      │            ─── Delete request ──► delete_reports (interrupt → confirm → delete/cancel)
      │ Data question
      ▼
┌──────────────────┐
...
### 7. High-Stakes Oversight - Delete Reports with interrupt() (Requirement 3)

File: `src/nodes/delete_reports.py`

Uses LangGraph's `interrupt()` to pause execution and ask user confirmation before destructive actions.
The node holds a mock `SAVED_REPORTS` list. When the router classifies intent as `"delete"`, the graph
routes to this node, which calls `interrupt()` to pause and return a confirmation prompt to the CLI.

The CLI loop in `main.py` detects `__interrupt__` in the result, displays the message, collects user
input, and resumes the graph with `Command(resume=answer)`. If "yes", reports are deleted. Otherwise cancelled.

This same `interrupt()` mechanism works automatically in LangGraph Studio (`langgraph dev`) — the UI
renders the interrupt as a message and provides an input field for the user response.
